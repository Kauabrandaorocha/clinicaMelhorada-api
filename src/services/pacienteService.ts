...
export const update = async (id: number, data: PacienteUpdateData): Promise<Paciente> => {
    return prisma.paciente.update({
        where: { id },
        data: { ...data, dataNascimento: data.dataNascimento ?  new Date(data.dataNascimento) : undefined},  
    });
};
